...
INDEX = [
    ...
    ('single-agent cost per question at 32-\nstep streams, vs Agora',
     '32 步流上单智能体每问成本（相对 Agora）'),
    ('single-agent cost per question at 32-step streams, vs Agora',
     '32 步流上单智能体每问成本（相对 Agora）'),
    ('corrupted boards caught, 1600 held-\nout questions', '被污染信念捕获率（1600 个留出问题）'),
    ('corrupted boards caught, 1600 held-out questions', '被污染信念捕获率（1600 个留出问题）'),
    ('vs the hand-set gate, at 0.9%\nfalse-fire', '优于手工门控，误触发仅 0.9%'),
