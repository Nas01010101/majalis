...
- **实时喂入你自己的证据。** 在[社会实况](http://47.237.187.157:8080/zh/live)中切换到"实况 — 试一试"：粘贴带日期的证据行，观看抽取器构建信念、学习世界模型实时重新打分、门控只在 P(wrong) 飙升处发起辩论——全部经由真实部署的社会。匿名访问共享每日小额预算（`AGORA_LIVE_DAILY_CAP`）；`X-Agora-Token` 可绕过限额。
