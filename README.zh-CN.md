...
Majalis 不是基准测试玩具——它是**让任何持续演化的信念库保持正确、且成本不随流长增长**的通用配方。
每种模式都对应一个已交付、已测量的功能面：

| 你在运行 | Majalis 消除的失败 | 已交付、已测量的功能面 |
|---|---|---|
| 由流式数据喂养的知识库 / wiki / CRM | 过期或被谣言顶替的事实被当作真相提供 | 维护模式：`majalis-maintain` 在维护窗口修复信念板——**112/112，零问答时辩论，$0.0092/题** |
| 尽调、事实核查、威胁情报分级 | 冲突来源凭感觉裁决 | 门控 + 案卷裁决：**86.2% 污染捕获率、0.9% 误触发**；投委会演示端到端 |
| 任何有 token 预算的多智能体部署 | 辩论成本随用量增长 | 平坦 **$0.005/题**、448/448 正确，对比单智能体线性增长（32 步时 2.5×）；朴素 MAD 12.6× |
| 必须约束错误率的流水线 | 用"LLM 看起来挺自信"当质检 | 保形接受：**E[error \| committed] ≤ α**，经验校验（n=1,600 上 2.1% ≤ 5%） |
| 你自己的证据流 | 以上全部，但是你的数据 | 已部署机器上的实时 `/ingest` + `/ask` API + 社会视图；任何记录运行皆可 `majalis replay` |
...
| 对比臂 | 准确率 | 每问成本 | 说明 |
|---|---|---|---|
| **Majalis**（学习门控，默认） | **448/448，全部流长度** | **$0.0049–0.0054/问，平坦** | 门控决策 0 次 LLM 调用；约 8% 的问题发起辩论 |
| Majalis（规划门控） | 320/320（20 个种子） | $0.0059/问 | 双分支 argmax；诚实零结果——与反应式门控持平，从不超越 |
| **Majalis（零延迟维护）** | **112/112 实测（7 个种子）** | $0.0092/问 | **问答期零辩论**——修复在批次之间对世界模型标记的键上进行 |
| Majalis（手工门控，可选） | 303/304，全部流长度 | $0.0056–0.0067 | 对比臂 `majalis`；对 6–16% 的问题辩论 |
| 单智能体 | 480/480 | $0.0079 → $0.0137，线性增长（32 步时 2.5×） | 每问重读整条流 |
| 朴素 MAD（3×3） | 32/32 | $0.0709 | 手工门控臂的 12.6 倍（$0.0056）；约为学习默认臂的 13 倍 |
| Majalis（去辩论消融） | 107/112 (95.5%) | $0.0058 | 其 5 个错误恰好是世界模型标记的谣言污染信念；每个门控/维护臂都能全部纠正 |
