...
**Objective**: minimize cost_usd per question of the `majalis` session arm.

**Metric** (runner-owned, fail-closed): `target.py` runs the majalis arm on
...
**Editable surface**: `target.py` ONLY (it sets the `MAJALIS_GATE_K` /
`MAJALIS_GATE_SKIP_DOUBT` env knobs). The bench, grader, stream generator, and
