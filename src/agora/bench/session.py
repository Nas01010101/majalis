...
def run_session_arm(arm: str, seed: int) -> dict:
    events = make_session(seed)
    t0 = time.monotonic()
    records = REPLAYS[arm](events, seed)
    latency = time.monotonic() - t0
    raw_path = RESULTS_DIR / "raw" / f"session_{arm}_s{seed}.jsonl"
    ...
    return {"arm": arm, "seed": seed, "n": len(records), "correct": correct,
...
def main() -> None:
    ...
    for seed in (int(s) for s in args.seeds.split(",")):
        for arm in args.arms.split(","):
            print(f"session: {arm} seed={seed} ...", flush=True)
            summaries.append(run_session_arm(arm, seed))

    out = RESULTS_DIR / "session_summary.json"
    existing = json.loads(out.read_text()) if out.exists() else []
    # Latest run wins per (arm, seed).
    merged = {(s["arm"], s["seed"]): s for s in existing}
    merged.update({(s["arm"], s["seed"]): s for s in summaries})
    out.write_text(json.dumps(list(merged.values()), indent=2))

    by_arm: dict[str, dict] = {}
    for s in merged.values():
        agg = by_arm.setdefault(s["arm"], {"n": 0, "correct": 0, "tokens": 0,
                                           "cost": 0.0})
        agg["n"] += s["n"]
        agg["correct"] += s["correct"]
        agg["tokens"] += s["tokens"]
        agg["cost"] += s["cost_usd"]
    print(f"\n{'arm':<8} {'accuracy (Wilson 95%)':<28} {'tok/q':>8} {'$/q':>9}")
    for arm, a in by_arm.items():
        print(f"{arm:<8} {fmt_acc(a['correct'], a['n']):<28} "
