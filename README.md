...
- **Retrain it yourself.** `python scripts/gen_wm_dataset.py && python train/train_wm.py` — ~2 min end to end (torch to train, numpy-only to serve). The replaced heuristic survives as an ablation (`AGORA_WM=heuristic`).
...
|---|---|---|---|
| **Agora** | **272/272, all stream lengths** | **$0.0056, flat in stream length** | debates only 6–16% of questions |
| single agent | 272/272 | $0.0079 → $0.0137, linear | re-reads the stream per question |
| vanilla MAD (3×3) | 32/32 | $0.0709 | 10.6× Agora's cost |
| Agora, debate ablated | 77/80 (96.2%) | $0.0060 | its 3 errors are exactly the rumor-poisoned beliefs the WM flagged; gated debate fixes all 3 for +$0.0004/q |
